...
#define NEIGHBOR_LIFETIME 180//00
...
print_list(list_t neighbor_list){
	...
	for(n = list_head(neighbor_list); n != NULL; n = list_item_next(n)) {
		PRINTF("address %d.%d count %d cost %f lseqn %d\n", n->addr.u8[1],
				n->addr.u8[0], n->count, n->cost, n->last_seq_no);
	}
}
/*---------------------------------------------------------------------------*/
...
void
c_lqe_ewma_open(struct pipe *p, struct stackmodule_i *module){
	PRINTF("c_lqe_ewma_open\n");
	seqno = 1;
	clock_init();
	ctimer_set(&t, CLOCK_SECOND * NEIGHBOR_LIFETIME, periodic, p);
}
...
void
c_lqe_ewma_close(struct pipe *p, struct stackmodule_i *module) {
	PRINTF("c_lqe_ewma_close\n");
}
/*---------------------------------------------------------------------------*/

int
c_lqe_ewma_send(struct pipe *p, struct stackmodule_i *module) {
	PRINTF("c_lqe_ewma_send \n");
...
void
c_lqe_ewma_sent(struct pipe *p, struct stackmodule_i *module) {
	PRINTF("c_lqe_ewma_sent\n");
}
